...
function perform_upsert(array){
    ...
    for(var i=0;i<array.length;i++){
        ...
        var pbt = NSOA.record.oaProjectBudgetTransaction();
        var currentbudgettotal = find_current_budget_total(pbt.project_budget_groupid, pbt);
        obj.userid = 183;//Default user for task assignments to be all grouped under one task.
